    ...
    <>
            ...
            {showEndBanner && (
              <div className="end-banner show" style={{margin:'0 16px 16px'}}>
                ...
                <div className="btn-row" style={{flexWrap:'wrap',gap:8}}>
                  ...
                  <button className="btn btn-primary" onClick={()=>setScreen('export')} style={{background:'linear-gradient(135deg,#cc0000,#880000)',border:'none',width:'100%'}}>▶ YouTube Export — Titles, Description</button>
                </div>
      ...
      {screen==='export' && (
        <div className="page-content" style={{background:'var(--void)'}}>
          <div className="topbar">
            <button className="btn btn-icon" onClick={()=>setScreen('story')}>←</button>
            <span className="topbar-logo" style={{fontFamily:"'Cinzel Decorative',serif",fontSize:13,color:'#ff4444'}}>▶ YOUTUBE EXPORT</span>
          </div>
          <div style={{flex:1,overflowY:'auto',padding:12,display:'flex',flexDirection:'column',gap:12}}>

            {/* Title Generator */}
            <ExportCard title="🎯 YouTube Title Generator" titleColor="#44bb66" bg="#000a00" borderColor="#003300">
              <div style={{fontSize:12,color:'#666',lineHeight:1.6,marginBottom:8}}>7 clickbait Hindi titles AI se — high CTR ke liye</div>
              <ExportTitles storyChunks={storyChunks} stateRef={stateRef} epNum={epNum} toast={toast}/>
            </ExportCard>

            {/* Description Generator */}
            <ExportCard title="📝 Description + Tags" titleColor="#4488ff" bg="#00000a" borderColor="#000033">
              ...
              <ExportDesc storyChunks={storyChunks} stateRef={stateRef} epNum={epNum} season={season} toast={toast}/>
            </ExportCard>

            {/* Thumbnail Enhancer */}
            <ExportCard title="🖼 Thumbnail Prompt Enhancer" titleColor="#cc88ff" bg="#0a000a" borderColor="#330033">
              <div style={{fontSize:12,color:'#666',lineHeight:1.6,marginBottom:8}}>Basic prompt → viral YouTube thumbnail prompt</div>
              <ExportThumb storyChunks={storyChunks} toast={toast}/>
            </ExportCard>
...
function ExportCard({ title, titleColor, bg, borderColor, children }) {
...
function ExportTitles({ storyChunks, stateRef, epNum, toast }) {
  const [loading, setLoading] = useState(false);
  const [titles,  setTitles]  = useState([]);
  async function generate() {
    if (!storyChunks.length) { toast('⚠️ Pehle story complete karo!'); return; }
  ...
  function copy(t) { navigator.clipboard.writeText(t).then(()=>toast('✅ Title copy ho gaya!')); }
  return (
    <>
      <button className="btn btn-primary" onClick={generate} disabled={loading}
        style={{background:'linear-gradient(135deg,#005500,#002200)',boxShadow:'0 4px 16px rgba(0,150,0,0.3)'}}>
        {loading?<><div className="spinner"/>Titles ban rahe hain...</>:'🎯 Titles Generate Karo'}
      </button>
      {titles.length>0 && (
        <div className="yt-output-card">
          {titles.map((t,i)=>(
            <div key={i} className="yt-title-option">
              ...
              <button className="yt-copy-btn" onClick={()=>copy(t)}>📋 Copy</button>
            </div>
          ))}
        </div>
      )}
...
function ExportDesc({ storyChunks, stateRef, epNum, season, toast }) {
  const [loading, setLoading] = useState(false);
  const [desc,    setDesc]    = useState('');
  async function generate() {
    if (!storyChunks.length) { toast('⚠️ Pehle story complete karo!'); return; }
    setLoading(true); setDesc('');
    const storyText = storyChunks.map(c=>c.text).join('\n\n').slice(0,800);
    const curTitle  = stateRef.current.title||'';
    try {
      const res  = await fetch('/api/ai',{ method:'POST', headers:{'Content-Type':'application/json'},
  ...
  function copy() { navigator.clipboard.writeText(desc).then(()=>toast('✅ Description copy ho gayi!')); }
  return (
    <>
      <button className="btn btn-primary" onClick={generate} disabled={loading}
        style={{background:'linear-gradient(135deg,#000055,#000033)',boxShadow:'0 4px 16px rgba(0,0,150,0.3)'}}>
        {loading?<><div className="spinner"/>Generating...</>:'📝 Description Generate Karo'}
      </button>
      {desc && (
        <div className="yt-output-card">
          <div className="yt-desc-box">{desc}</div>
          <button className="yt-copy-btn" onClick={copy}>📋 Copy Karo</button>
        </div>
      )}
    </>
  );
...
function ExportThumb({ storyChunks, toast }) {
  const [input,   setInput]   = useState('');
  const [loading, setLoading] = useState(false);
  const [result,  setResult]  = useState('');
  async function enhance() {
    const base = input || storyChunks.map(c=>c.text).join(' ').slice(0,300);
  ...
  function copy() { navigator.clipboard.writeText(result).then(()=>toast('✅ Thumbnail prompt copy ho gaya!')); }
  return (
    <>
      ...
      <button className="btn btn-primary" onClick={enhance} disabled={loading}
        style={{background:'linear-gradient(135deg,#550055,#330033)',boxShadow:'0 4px 16px rgba(150,0,150,0.3)'}}>
        {loading?<><div className="spinner"/>Enhancing...</>:'🖼 Thumbnail Prompt Enhance Karo'}
      </button>
      {result && (
        <div className="yt-output-card">
          <div className="yt-enhanced-box">{result}</div>
          <button className="yt-copy-btn" onClick={copy}>📋 Copy Karo</button>
        </div>
      )}
    </>
